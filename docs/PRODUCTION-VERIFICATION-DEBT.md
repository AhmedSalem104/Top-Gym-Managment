...
| 4 | Real private Object Storage provider for proofs/backups/branding/exports | `REQUIRES PRODUCTION VERIFICATION` | Provider-neutral integration is implemented; MinIO/private bucket/application policy are provisioned on the VPS, but the API remains localhost-only until an HTTPS hostname, TLS and Vercel secret configuration are completed. Existing SQL-backed rows remain readable; new external uploads fail closed until activation. | Configure DNS/TLS/firewall, add the restricted application credential directly to Vercel, then test private keys, signed URLs, MIME/size limits, tenant prefixes, deletion and recovery on an isolated environment. | High. |
