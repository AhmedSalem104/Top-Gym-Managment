...
| 8 | Central log shipping, alert delivery and incident correlation | `REQUIRES PRODUCTION VERIFICATION` | Local safe logs and request instrumentation exist, but provider alert routing is not proven. | Trigger a synthetic error in Staging/preview, confirm request ID correlation and alert routing without sensitive data. | High. |
