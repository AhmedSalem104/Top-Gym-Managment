...
function registerMemberSubscriptionRoutes(app, { service, asyncRoute, ownerOnly }) {
    ...
    app.post('/api/member-portal/subscription-requests', asyncRoute(controller.createPortalRequest));
    app.post('/api/member-portal/subscription-requests/:requestId/proof', express.raw({ type: 'application/octet-stream', limit: '4mb' }), asyncRoute(controller.uploadPortalProof));
