...
function createIntelligenceController({ intelligenceService }) {
    return {
        overview: async (request, response) => {
            ...
            response.json(await intelligenceService.getOverview({ actorUserId: request.auth?.id }));
        },
        ...
        churn: async (request, response) => {
            response.set('Cache-Control', 'private, max-age=15, stale-while-revalidate=60');
            response.json(await intelligenceService.getChurnRisks({ limit: request.query.limit, actorUserId: request.auth?.id }));
        },
