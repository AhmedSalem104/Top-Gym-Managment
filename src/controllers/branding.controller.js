    ...
    return {
        publicBranding: async (request, response) => {
            ...
            const result = platformScope
                ...
                : await brandingService.getPublicBranding({ readOnly: request.readOnlyRequest });
            response.set('Cache-Control', 'no-store, no-cache, must-revalidate, private');
            response.json(result);
        },

        settings: async (request, response) => {
            response.json(await brandingService.ownerResponse({ readOnly: request.readOnlyRequest }));
        },
        ...
        publishedAsset: async (request, response) => {
            const asset = await brandingService.readAsset(request.params.key, 'published', { readOnly: request.readOnlyRequest });
        ...
        draftAsset: async (request, response) => {
            const asset = await brandingService.readAsset(request.params.key, 'draft', { readOnly: request.readOnlyRequest });
            if (!asset) return response.status(404).end();
            response.set({
