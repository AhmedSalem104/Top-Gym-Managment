...
function createBranchController({ branchService }) {
    ...
    return {
        ...
        bootstrap: async (request, response) => response.json(await branchService.bootstrap({ userId: request.auth?.id, role: request.auth?.role, readOnly: request.readOnlyRequest })),
