...
function usageDto(usage, entitlements) {
    ...
    const rows = [
        [isTrainer ? 'clients' : 'members', isTrainer ? 'Clients' : 'Members', isTrainer ? limits.maxClients : limits.maxMembers],
...
async function updateOverrides(tenantId, body = {}, actorUserId, meta = {}) {
    ...
    const values = {
        ...
        maxClients: positiveOrNull(body.maxClients, 'Clients limit'),
    ...
    const clear = bool(body.clear) || (!values.maxMembers && !values.maxClients && !values.maxUsers && !values.maxAiGenerations && !values.maxStorageMb && !values.maxBranches && !Object.keys(values.features).length && !values.notes);
    const pool = await getPool();
    if (clear) {
        await pool.request().input('tenantId', sql.Int, id).query('DELETE FROM dbo.saas_tenant_overrides WHERE tenant_id=@tenantId;');
    } else {
        await pool.request().input('tenantId', sql.Int, id).input('maxMembers', sql.Int, values.maxMembers).input('maxClients', sql.Int, values.maxClients).input('maxUsers', sql.Int, values.maxUsers).input('maxAiGenerations', sql.Int, values.maxAiGenerations).input('maxStorageMb', sql.Int, values.maxStorageMb).input('maxBranches', sql.Int, values.maxBranches).input('features', sql.NVarChar(sql.MAX), JSON.stringify(values.features)).input('notes', sql.NVarChar(1000), values.notes || null).input('actorId', sql.Int, actorUserId == null ? null : Number(actorUserId)).query(`IF EXISTS (SELECT 1 FROM dbo.saas_tenant_overrides WHERE tenant_id=@tenantId)
            UPDATE dbo.saas_tenant_overrides SET max_members=@maxMembers,max_clients=@maxClients,max_users=@maxUsers,max_ai_generations=@maxAiGenerations,max_storage_mb=@maxStorageMb,max_branches=@maxBranches,features_json=@features,notes=@notes,updated_by_user_id=@actorId,updated_at=SYSUTCDATETIME() WHERE tenant_id=@tenantId;
        ELSE
            INSERT INTO dbo.saas_tenant_overrides (tenant_id,max_members,max_clients,max_users,max_ai_generations,max_storage_mb,max_branches,features_json,notes,created_by_user_id,updated_by_user_id) VALUES (@tenantId,@maxMembers,@maxClients,@maxUsers,@maxAiGenerations,@maxStorageMb,@maxBranches,@features,@notes,@actorId,@actorId);`);
    }
