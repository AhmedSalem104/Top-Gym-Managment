/*
    Notification read-state cascade compatibility fix.

    SQL Server rejects the historical CASCADE on
    FK_saas_notification_reads_user because a tenant deletion can reach the
    read row through both gym_users and saas_notifications. Notification
    deletion owns read-state cleanup; user deletion remains NO ACTION.

    This migration is idempotent and also bootstraps the notification core for
    a fresh local database before the historical 031 migration is replayed by
    the local bootstrap runner. Production release paths apply it after 031.
*/
...
IF EXISTS (
    ...
    WHERE name=N'FK_saas_notification_reads_user'
      AND parent_object_id=OBJECT_ID(N'dbo.saas_notification_reads')
      AND delete_referential_action_desc <> N'NO_ACTION'
)
BEGIN
    EXEC(N'
        ALTER TABLE dbo.saas_notification_reads DROP CONSTRAINT FK_saas_notification_reads_user;
        ALTER TABLE dbo.saas_notification_reads ADD CONSTRAINT FK_saas_notification_reads_user
            FOREIGN KEY (user_id) REFERENCES dbo.gym_users(id) ON DELETE NO ACTION;
    ');
END;
