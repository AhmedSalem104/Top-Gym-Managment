        ...
        window.addEventListener('logicfit:phone-formatter-ready', () => {
            ...
            const detailsId = $('detailsDialog')?.dataset.memberId;
            if (detailsId) {
                const member = (state.members || []).find((item) => String(item.id) === String(detailsId));
                if (member) updateDetailsPhoneDisplay(member);
            }
        });
        ...
        function openDialog(action, member) {
            ...
            const freezeWhatsappOption = $('dialogFreezeWhatsappOption');
            if (freezeWhatsappOption) {
                freezeWhatsappOption.hidden = action !== 'freeze';
                if (action === 'freeze') $('dialogFreezeSendWhatsApp').checked = true;
            }
        ...
        async function openRefundDialog(member) {
            ...
            $('dialogDescription').textContent = `${member.fullName} · ${displayPhone(member.phone, member.phoneCountry)}`;
        ...
        function closeDialog() { const dialog = $('actionDialog'); closeDialogSafely(dialog); state.dialogAction = null; state.dialogMember = null; state.refundPreview = null; const freezeWhatsappOption = $('dialogFreezeWhatsappOption'); if (freezeWhatsappOption) freezeWhatsappOption.hidden = true; const freezeWhatsappInput = $('dialogFreezeSendWhatsApp'); if (freezeWhatsappInput) freezeWhatsappInput.checked = true; $('dialogSubmit').disabled = false; $('dialogSubmit').textContent = '\u062a\u0623\u0643\u064a\u062f'; }
        ...
        function updateDetailsPhoneDisplay(member) {
            if (!member) return;
            const formatted = displayPhone(member.phone, member.phoneCountry);
            $('detailsSubtitle').textContent = `${member.fullName || 'تفاصيل العميل'} · ${formatted}${member.email ? ` · ${member.email}` : ''}`;
        }
        ...
        async function openDetails(member) {
            ...
            $('detailsSubtitle').textContent = member.fullName || 'تفاصيل العميل';
            $('detailsContent').innerHTML = '<div class="loading">جاري تحميل التفاصيل…</div>';
            openDialogSafely(dialog);
            await window.LogicFitPhoneInputs?.loadFormatter?.();
            updateDetailsPhoneDisplay(member);
            try {
                const cached = detailsCache.get(member.id);
                if (cached) {
                    renderDetails(cached); updateDetailsPhoneDisplay(cached.member || member); renderPaymentHistory(cached);
                    window.dispatchEvent(new CustomEvent('topgym:member-details-opened', { detail: { member, details: cached } }));
                    updateDetailsPhoneDisplay(cached.member || member);
                }
                else {
                    ...
                    updateDetailsPhoneDisplay(details.member || member);
        ...
        // app.js is the authenticated application controller. It may be
        // loaded after the first paint by app-shell-bootstrap, so support
        // both the classic deferred entry and dynamic loading safely.
