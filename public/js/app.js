        ...
        const PAYMENT_LABELS = { cash: 'نقدي', card: 'بطاقة', transfer: 'تحويل', other: 'أخرى' };
         ...
         const ACTION_LABELS = { details: 'التفاصيل', edit: 'تعديل العضو', renew: 'تجديد الاشتراك', freeze: 'تجميد العضوية', resume: 'استئناف العضوية', payment: 'تسجيل دفعة', delete: 'حذف العضو' };
         const ACTION_PERMISSIONS = { details: 'members.read', edit: 'members.write', renew: 'members.write', freeze: 'members.write', resume: 'members.write', payment: 'members.write', delete: 'members.delete' };
         const ACTION_ICONS = {
         ...
         function actionButton(action, memberId, classes = 'btn btn-light btn-small', extra = '') { const label = ACTION_LABELS[action] || action; const permission = ACTION_PERMISSIONS[action] || ''; return `<button class="${classes} icon-action rounded-lg shadow-none transition-colors" data-action="${action}" data-label="${label}" data-permission="${permission}" data-id="${memberId}" aria-label="${label}" title="${label}" type="button" ${extra}>${actionIcon(action)}</button>`; }
         ...
         const state = { members: [], dashboard: null, pricing: DEFAULT_PRICING, editing: null, dialogAction: null, dialogMember: null, endDateManual: false };
        ...
        async function loadData() {
            return withLoader(async () => {
                ...
                const pricingRequest = api('/api/pricing');
                ...
                try {
                    ...
                    renderMembers();
                    updateFormPricing();
                    const [dashboard, pricing] = await Promise.all([dashboardRequest, pricingRequest]);
                    ...
                    if (error.message.includes('تسجيل الدخول') || error.message.includes('صلاحية')) window.topGymAuth?.handleUnauthorized();
        ...
        async function loadMembersOnly() {
            ...
            return withLoader(async () => {
                ...
                catch (error) { if (error.name !== 'AbortError') { $('membersList').innerHTML = `<div class="empty">${escapeHtml(error.message)}</div>`; if (error.message.includes('تسجيل الدخول') || error.message.includes('صلاحية')) window.topGymAuth?.handleUnauthorized(); else await notify(error.message, 'error'); } }
        ...
        function openDialog(action, member) {
            ...
            if (action === 'freeze') fields.innerHTML = `<div class="freeze-limit-note"><span class="freeze-limit-icon">${actionIcon('freeze')}</span><div><strong>التجميد المستخدم ${freezeCount}/${freezeLimit}</strong><span>متبقي ${Math.max(0, freezeLimit - freezeCount)} مرات لهذا العضو.</span></div></div><div class="field"><label for="dialogDays">عدد أيام التجميد</label><input id="dialogDays" type="number" min="1" max="365" value="7" required></div><div class="field"><label for="dialogReason">السبب (اختياري)</label><textarea id="dialogReason" maxlength="500"></textarea></div>`;
             if (action === 'renew' || action === 'payment') { const plan = sub.plan || 'gym_only'; const type = sub.type || activeTypeEntries()[0]?.[0] || 'monthly'; const discount = sub.discountAmount || 0; const paidDefault = action === 'renew' ? 0 : (sub.amountPaid || 0); const planOptions = Object.entries(state.pricing.plans).map(([code, item]) => `<option value="${escapeHtml(code)}">${escapeHtml(item.label)}</option>`).join(''); const typeOptions = activeTypeEntries().map(([code, item]) => `<option value="${escapeHtml(code)}">${escapeHtml(item.label)}</option>`).join(''); fields.innerHTML = `<div class="field-grid"><div class="field"><label for="dialogPlan">الباقة</label><select id="dialogPlan">${planOptions}</select></div><div class="field"><label for="dialogType">نوع العضوية</label><select id="dialogType">${typeOptions}</select></div></div><div class="field-grid"><div class="field"><label for="dialogDiscount">الخصم</label><input id="dialogDiscount" type="number" min="0" step="0.01" value="${discount}"></div><div class="field"><label for="dialogDue">المستحق بعد الخصم</label><input id="dialogDue" type="number" readonly></div></div><div class="pricing-summary" id="dialogPricing"></div><div class="field"><label for="dialogPaid">إجمالي المدفوع</label><input id="dialogPaid" type="number" min="0" step="0.01" value="${paidDefault}" required></div><div class="field"><label for="dialogMethod">طريقة الدفع</label><select id="dialogMethod"><option value="cash">نقدي</option><option value="card">بطاقة</option><option value="transfer">تحويل</option><option value="other">أخرى</option></select></div>`; $('dialogPlan').value = plan; $('dialogType').value = type; $('dialogMethod').value = sub.paymentMethod || 'cash'; updateDialogPricing(); $('dialogPlan').addEventListener('change', updateDialogPricing); $('dialogType').addEventListener('change', updateDialogPricing); $('dialogDiscount').addEventListener('input', updateDialogPricing); }
        ...
        async function submitDialog(event) { event.preventDefault(); const action = state.dialogAction; const member = state.dialogMember; if (!action || !member) return; if (action === 'freeze' && Number(member.membership?.freezeCount || 0) >= Number(member.membership?.freezeLimit || FREEZE_LIMIT)) { await notify(`تم استهلاك الحد الأقصى للتجميد (${FREEZE_LIMIT} مرات) لهذا العضو.`, 'warning'); return; } try { await withLoader(async () => { if (action === 'freeze') await api(`/api/members/${member.id}/freeze`, { method: 'POST', body: JSON.stringify({ days: Number($('dialogDays').value), reason: $('dialogReason').value }) }); if (action === 'renew') await api(`/api/members/${member.id}/renew`, { method: 'POST', body: JSON.stringify({ membershipType: $('dialogType').value, membershipPlan: $('dialogPlan').value, discountAmount: Number($('dialogDiscount').value || 0), amountPaid: Number($('dialogPaid').value || 0), paymentMethod: $('dialogMethod').value }) }); if (action === 'payment') { const pricing = calculateClientPricing($('dialogType').value, $('dialogPlan').value, $('dialogDiscount').value); await api(`/api/memberships/${member.membership.id}/payments`, { method: 'POST', body: JSON.stringify({ listPrice: pricing.listPrice, discountAmount: pricing.discountAmount, amountPaid: Number($('dialogPaid').value || 0), paymentMethod: $('dialogMethod').value }) }); } }, 'جاري تنفيذ الإجراء…'); closeDialog(); await refreshAfterAction(action === 'freeze' ? 'تم تجميد العضوية.' : action === 'renew' ? 'تم تجديد العضوية.' : 'تم تحديث الدفع.'); } catch (error) { await notify(error.message, 'error'); } }
         ...
         async function openPricingDialog() { try { await withLoader(async () => { state.pricing = await api('/api/pricing'); syncPlanOptions(); renderPricingTable(); }, 'جاري تحميل أسعار الباقات TOP GYM…'); const dialog = $('pricingDialog'); if (typeof dialog.showModal === 'function') dialog.showModal(); else dialog.setAttribute('open', ''); } catch (error) { await notify(error.message, 'error'); } }
         async function openMembershipTypesDialog() { try { await withLoader(async () => { state.pricing = await api('/api/pricing'); syncPlanOptions(); renderMembershipTypesTable(); }, 'جاري تحميل أنواع العضويات TOP GYM…'); const dialog = $('membershipTypesDialog'); if (typeof dialog.showModal === 'function') dialog.showModal(); else dialog.setAttribute('open', ''); } catch (error) { await notify(error.message, 'error'); } }
        ...
        function renderPaymentHistory(data) {
            ...
            section.innerHTML = `<h4>سجل المدفوعات</h4>${payments.length ? `<div class="history-scroll"><table class="history-table"><thead><tr><th>التاريخ</th><th>الاشتراك</th><th>المستحق</th><th>المدفوع</th><th>المتبقي</th><th>طريقة الدفع</th></tr></thead><tbody>${payments.map((payment) => `<tr><td>${formatDate(payment.paidAt || payment.createdAt)}</td><td>${escapeHtml(planLabel(payment.plan))}<span class="table-sub">${escapeHtml(typeLabel(payment.type))}</span></td><td>${money(payment.amountDue)}</td><td>${money(payment.amountPaid)}</td><td><span class="table-money${payment.amountRemaining > 0 ? ' has-debt' : ''}">${money(payment.amountRemaining)}</span></td><td>${PAYMENT_LABELS[payment.paymentMethod] || escapeHtml(payment.paymentMethod || '—')}</td></tr>`).join('')}</tbody></table></div>` : '<div class="history-empty">لا توجد مدفوعات مسجلة بعد.</div>'}`;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.addEventListener('topgym:auth-ready', () => loadData());
            loadData();
        });
