(() => {
  ...
  function renderFilters() {
    ...
    return `<div class="portal-library-filter-bar"><label><span>بحث سريع</span><input class="search-input" id="portalLibrarySearch" type="search" value="${escapeHtml(state.search)}" placeholder="ابحث باسم التمرين أو الطعام..." autocomplete="off"></label>${state.activeType === 'foods' ? `<label><span>التصنيف</span><select class="form-control" id="portalFoodCategory"><option value="">كل التصنيفات</option>${(filters.categories || []).map((item) => `<option value="${escapeHtml(item)}"${item === state.category ? ' selected' : ''}>${escapeHtml(item)}</option>`).join('')}</select></label>` : `<label><span>المستوى</span><select class="form-control" id="portalExerciseDifficulty"><option value="">كل المستويات</option>${(filters.difficulties || []).map((item) => `<option value="${escapeHtml(item)}"${item === state.difficulty ? ' selected' : ''}>${escapeHtml(translate(item, 'difficulty'))}</option>`).join('')}</select></label><label><span>الأداة</span><select class="form-control" id="portalExerciseEquipment"><option value="">كل الأدوات</option>${(filters.equipment || []).map((item) => `<option value="${escapeHtml(item)}"${item === state.equipment ? ' selected' : ''}>${escapeHtml(translate(item, 'equipment'))}</option>`).join('')}</select></label>`}</div>`;
  }
