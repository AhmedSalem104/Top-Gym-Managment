            ...
            const enhanceDailyAlerts = () => {
                alertsList.querySelectorAll('.alert-card:not([data-alert-enhanced])').forEach((card) => {
                    ...
                    const quickAction = memberId
                        ? `<button type="button" class="alert-whatsapp-button" data-alert-whatsapp="${kind}" data-member-id="${escape(memberId)}" data-alert-name="${escape(card.dataset.alertName || name)}" data-alert-phone="${escape(card.dataset.alertPhone || phone)}" data-alert-status="${escape(card.dataset.alertStatus || status)}" data-alert-end="${escape(card.dataset.alertEnd || '')}" data-alert-freeze-end="${escape(card.dataset.alertFreezeEnd || '')}" data-alert-remaining="${escape(card.dataset.alertRemaining || '')}" data-alert-days="${escape(card.dataset.alertDays || '')}"><svg class="ui-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 11.5a8 8 0 0 1-8 8 8.5 8.5 0 0 1-3.7-.85L4 20l1.35-4.05A8.5 8.5 0 1 1 20 11.5Z"/><path d="M8.7 9.1c.2-.45.4-.46.7-.47h.35c.2 0 .4.08.5.34l.65 1.5c.1.23.08.42-.08.62l-.42.52c.55 1.1 1.4 1.8 2.55 2.3l.45-.5c.17-.2.36-.23.6-.14l1.42.63c.25.12.34.3.31.55-.1.8-.68 1.35-1.47 1.4-2.3.12-5.98-3.5-6.1-6.75-.02-.01.17-.75.54-1.02Z"/></svg><span>واتساب يدوي</span></button>`
                        : '';
