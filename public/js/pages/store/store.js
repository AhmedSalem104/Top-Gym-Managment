(() => {
    ...
    function printReceipt(sale) {
        ...
        popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${esc(sale.saleNumber)}</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#0f172a}h1{text-align:center;color:#2563eb}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:center}.total{display:flex;justify-content:space-between;font-weight:700;font-size:18px;margin-top:18px}@media print{button{display:none}}</style></head><body><h1>TOP GYM</h1><p>إيصال مبيعات: ${esc(sale.saleNumber)}</p><table><thead><tr><th>المنتج</th><th>الكمية</th><th>الإجمالي</th></tr></thead><tbody>${lines}</tbody></table><div class="total"><span>الإجمالي</span><span>${money(sale.totalAmount)}</span></div><p>شكرًا لزيارتكم</p><script>window.onload=()=>window.print();</script></body></html>`);
