(() => {
    ...
    function refreshReportPhoneDisplays() {
        document.querySelectorAll('.reports-members-card tbody tr > td:first-child > small').forEach((node) => {
            const text = String(node.textContent || '');
            const separator = ' · ';
            const separatorIndex = text.lastIndexOf(separator);
            if (separatorIndex > -1) {
                const prefix = text.slice(0, separatorIndex);
                const phone = text.slice(separatorIndex + separator.length);
                node.textContent = `${prefix}${separator}${displayPhone(phone)}`;
            } else {
                node.textContent = displayPhone(text);
            }
        });
    }
    ...
    function renderFinance() {
        ...
        const paymentRows = payments.map((item) => `<tr><td><strong>${escapeHtml(item.fullName)}</strong><small>${escapeHtml(displayPhone(item.phone, item.phoneCountry) || '—')}</small></td><td>${reportBadge(item.transactionType, TRANSACTION_LABELS, 'transaction')}</td><td>${dateOnly(item.date)}</td><td>${escapeHtml(label(PAYMENT_LABELS, item.paymentMethod))}</td><td class="paid">${money(item.amountPaid)}</td><td>${money(item.amountRemaining)}</td></tr>`).join('');
        ...
        const summary = data.summary || {};
        const netMeta = Number(summary.refunds || 0) > 0 ? 'التحصيل − المصروفات − الاستردادات' : 'التحصيل − المصروفات';
        const kpis = `<div class="reports-kpis reports-kpis-compact">${reportKpi('التحصيل الفعلي', money(summary.collected), `${number(summary.paidTransactions)} دفعة · خلال الفترة`, 'green', 'payment')}${reportKpi('المصروفات', money(summary.expenses), `${number(summary.expensesCount)} مصروف · خلال الفترة`, 'amber', 'wallet')}${reportKpi('الصافي', money(summary.net), netMeta, 'teal', 'chart')}</div>`;
    ...
    function renderCoaching() {
        ...
        const workoutRows = programs.map((item) => `<tr><td><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.fullName)} · ${escapeHtml(displayPhone(item.phone, item.phoneCountry) || '—')}</small></td><td>${dateOnly(item.startDate)}<small>حتى ${dateOnly(item.endDate)}</small></td><td>${reportBadge(item.status, COACHING_STATUS_LABELS)}</td><td>${number(item.routines)} أيام</td><td>${number(item.exercises)} تمارين</td><td><div class="reports-coaching-actions"></div></td></tr>`).join('');
        const dietRows = diets.map((item) => `<tr><td><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.fullName)} · ${escapeHtml(displayPhone(item.phone, item.phoneCountry) || '—')}</small></td><td>${dateOnly(item.startDate)}<small>حتى ${dateOnly(item.endDate)}</small></td><td>${reportBadge(item.status, COACHING_STATUS_LABELS)}</td><td>${number(item.targetCalories)} سعر</td><td>${number(item.meals)} وجبات · ${number(item.foods)} أطعمة</td><td><div class="reports-coaching-actions"></div></td></tr>`).join('');
    ...
    function renderAttendance() {
        ...
        const memberRows = members.slice(0, 100).map((member) => `<tr><td><strong>${escapeHtml(member.fullName)}</strong><small>${escapeHtml(displayPhone(member.phone, member.phoneCountry) || '—')}</small></td><td>${number(member.visits)}</td><td>${number(member.totalMinutes)} دقيقة</td><td>${dateOnly(member.lastVisitDate)}</td></tr>`).join('');
        const absentRows = absent.slice(0, 100).map((member) => `<tr><td><strong>${escapeHtml(member.fullName)}</strong><small>${escapeHtml(displayPhone(member.phone, member.phoneCountry) || '—')}</small></td><td>${dateOnly(member.membershipEndDate)}</td><td><a class="btn btn-light btn-small" href="tel:${escapeHtml(member.phone)}">اتصال</a></td></tr>`).join('');
