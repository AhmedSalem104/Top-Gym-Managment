    ...
    return {
        ...
        async install() {
            ...
            await page.route('**/api/**', async (route) => {
                ...
                if (pathname === '/api/trainer/library/options') return body({ counts: { exercises: 1, foods: 1, muscles: 1 }, filters: {} });
                if (pathname === '/api/trainer/library/exercises') return body({ items: [{ id: 1, nameAr: 'Squat', name: 'Squat', targetMuscleNameAr: 'Legs', bodyPart: 'Legs', equipment: 'Barbell', difficulty: 'Intermediate' }], pagination: { page: 1, totalPages: 1, totalItems: 1 } });
                if (pathname === '/api/trainer/library/muscles') return body({ items: [{ id: 1, nameAr: 'Quadriceps', name: 'Quadriceps', bodyPart: 'Legs', descriptionAr: 'عضلة أمامية' }], pagination: { page: 1, totalPages: 1, totalItems: 1 } });
                if (pathname === '/api/trainer/library/foods') return body({ items: [{ id: 1, nameAr: 'Oats', nameEn: 'Oats', calories: 380, protein: 13, carbs: 68, fat: 7, servingSize: 100, servingUnit: 'g', category: 'grains' }], pagination: { page: 1, totalPages: 1, totalItems: 1 } });
...
test('Trainer Studio V2 renders a separate shell and navigates supported routes', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('[data-trainer-library-type="exercises"]')).toBeVisible();
    await expect(page.locator('[data-trainer-library-tab]')).toHaveCount(0);
    await expect(page.locator('.trainer-library-visual-card--exercises')).toHaveCount(1);
    await clickStudioRoute(page, 'muscles');
    await expect(page).toHaveURL(/\/trainer-workspace\/muscles$/);
    await expect(page.locator('[data-trainer-library-type="muscles"]')).toBeVisible();
    await expect(page.locator('.trainer-library-visual-card--muscles')).toHaveCount(1);
    await clickStudioRoute(page, 'foods');
    await expect(page).toHaveURL(/\/trainer-workspace\/foods$/);
    await expect(page.locator('[data-trainer-library-type="foods"]')).toBeVisible();
    await expect(page.locator('.trainer-library-visual-card--foods')).toHaveCount(1);
