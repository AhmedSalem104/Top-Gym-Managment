...
`X-TOP-GYM-RESTORE-CONFIRM: RESTORE`. يتم الاسترجاع داخل transaction واحدة، وتُحفظ نتيجة التنزيل والفحص والاسترجاع في `gym_backup_operations`. النسخ اليدوية تُنزّل مباشرة، بينما النسخة اليومية المجدولة تُحفظ مؤقتًا داخل `gym_backup_archives` لمدة يومين فقط.
