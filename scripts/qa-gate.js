...
function checkPlatformAdminSurface() {
    ...
    record('PLATFORM-ADMIN-PLAN-SAFETY', saasService.includes('DUPLICATE_PLAN_CODE') && saasService.includes('LAST_ACTIVE_PLAN') && saasService.includes('action: `plan_${nextStatus}`') && saasService.includes("return setPlanStatus(id, 'archived'") && client.includes('data-plan-delete'), 'plan validation, last-active protection, audited lifecycle changes and soft deletion are present', 'P0');
...
function checkProductionClosureContracts() {
    ...
    const reportDayPassReadOnly = reportService.includes('dayPassRepository.getRangeData({ fromDate: range.from, nextDate: range.nextDate, readOnly })')
        || reportService.includes('dayPassRepository.getRangeData({ fromDate: range.from, nextDate: range.nextDate, readOnly, branchId, sectionId })')
        || reportService.includes('dayPassRepository.getRangeData({ fromDate: range.from, nextDate: range.nextDate, readOnly, ...scope })');
    record('SEC-SUBSCRIPTION-REPORT-READONLY-PATH', memberSubscriptionService.includes('includeMemberCode = false, readOnly = false') && memberSubscriptionService.includes('ensureTables({ readOnly })') && memberSubscriptionService.includes('readOnly: Boolean(options.readOnly)') && memberSubscriptionController.includes('readOnly: request.readOnlyRequest') && reportDayPassReadOnly && reportService.includes('alertContactService.getLatestForAlerts(debtAlertSnapshots, { readOnly })'), 'subscription-request and report GET paths propagate read-only mode through storage guards and dependent reads', 'P1');
...
function checkPrintAndLazyLoadingSurface() {
    ...
    record('UI-CACHE-BUST', [
        /\/js\/app-shell-bootstrap\.js\?v=[^"'\s]+/u,
