...
function checkProductionClosureContracts() {
    ...
    record('SEC-FINANCE-STORE-READONLY-PATH', financeService.includes('async function getMonthlyFinance({ readOnly = false } = {})') && financeService.includes('ensureExpensesTable({ readOnly })') && financeController.includes('getMonthlyFinance({ readOnly: request.readOnlyRequest })') && dayPassService.includes('readOnly: Boolean(query.readOnly)') && dayPassRepository.includes('async function getRangeSummary({ fromDate, nextDate, readOnly = false })') && storeService.includes('async function getDashboard({ from, to, includeProfit = false, readOnly = false } = {})') && storeController.includes('readOnly: request.readOnlyRequest'), 'finance, day-pass and store GET paths propagate read-only mode through their schema guards', 'P1');
